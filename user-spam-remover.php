...
/*  Copyright 2010-2024 Joel Hardi
...
class UserSpamRemover {
  ...
  private function __construct(wpdb $wpdb) {
    self::$defaults['logDir'] = !empty(ini_get('error_log')) && is_dir(dirname(ini_get('error_log'))) && is_writeable(dirname(ini_get('error_log'))) ? dirname(ini_get('error_log')) : dirname(get_home_path());
...
?>
    ...
    <p><strong>Note:</strong> The first time you install this plugin, be sure to set the <strong>Log Directory</strong> below to a filesystem location that is <strong>webserver-writable</strong> and <strong>private (not visible over the web).</strong> You don't want people downloading your user data! (If you don't set a location, the plugin defaults to the PHP error_log directory if set, or else the directory one up from the WordPress root.)</p>
    <table class="form-table">
